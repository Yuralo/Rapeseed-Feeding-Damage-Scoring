...
def load_prepared_manifest(config: Config) -> list[dict[str, str]]:
    ...
    bad_modes = sorted({row["input_mode"] for row in rows} - ALLOWED_INPUT_MODES)
    if bad_modes:
    ...
    for row in rows:
        ...
        expected_grid_status = "true" if row["input_mode"].startswith("grid_") else "not_applicable"
        if row["grid_valid"].casefold() != expected_grid_status:
            raise ValueError(
                f"Prepared manifest has inconsistent route metadata for {row['image_id']}: "
                f"mode={row['input_mode']!r}, grid_valid={row['grid_valid']!r}"
            )
    ...
    if not prepared_ids <= source_ids:
        ...
        raise ValueError(
            ...
            f"extra={len(extra_ids)}. Re-run prepare_inputs without --limit."
        )
    ...
    absent = [row["prepared_path"] for row in rows if not Path(row["prepared_path"]).is_file()]
    if absent:
        raise FileNotFoundError(
            f"{len(absent)} prepared adaptation image(s) are missing. First paths:\n"
            + "\n".join(absent[:5])
        )
    return rows
