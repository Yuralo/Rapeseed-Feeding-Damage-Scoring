"""Train source-routed DINOv3 LoRA adaptation against a frozen teacher."""
...
def evaluate(model, loader, config: Config, device) -> dict[str, object]:
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            input_modes.update(batch["input_mode"])
            source_folders.update(batch["source_folder"])
            samples += batch_size
    if not samples:
...
def run(config: Config, resume: str | Path | None = None) -> dict:
    ...
    split_summary = {
        ...
        "training_input_modes": dict(Counter(row["input_mode"] for row in training)),
        "validation_input_modes": dict(Counter(row["input_mode"] for row in validation)),
        "source_routes": dict(
            Counter(f"{row['source_folder']}|{row['input_mode']}" for row in rows)
        ),
    ...
    summary = {
        ...
        "stopped_early": stopped_early,
        "input_modes": split_summary["input_modes"],
        "source_routes": split_summary["source_routes"],
        "validation": final_metrics,
        "model_parameters": parameters,
        "next_step": "Export the best checkpoint and run the controlled supervised MIL comparison.",
    }
