"""Train the 3x3-context plus 4x4/5x5-attention MIL head."""
...
from .model import TriScaleTiledMILRegressor
...
def empty_history() -> dict[str, list]:
    return {
        ...
        "val_mae": [],
        "val_r2": [],
        "val_regional_attention_entropy": [],
        "val_regional_top_tile_mass": [],
        "val_local_attention_entropy": [],
        "val_local_top_tile_mass": [],
        "learning_rates": [],
        "epoch_seconds": [],
    }
...
def run(config: Config, resume: str | Path | None = None) -> dict:
    ...
    model = TriScaleTiledMILRegressor(feature_dim, config).to(device)
    ...
    print(
        ...
        f"3x3 views={1 + config.context.rows * config.context.columns} | "
        f"4x4 views={1 + config.regional.rows * config.regional.columns} | "
        f"5x5 views={1 + config.local.rows * config.local.columns}",
        flush=True,
    )
    print(
        f"Trainable tri-scale head: {parameter_summary['trainable_parameters']:,} parameters | "
    ...
    for epoch in range(start_epoch, config.training.epochs + 1):
        ...
        for batch in train_loader:
            ...
            predictions = model(context, regional, local)
        ...
        if should_evaluate:
            ...
            regional_attention = attention["regional_4x4"]
            local_attention = attention["local_5x5"]
            history["val_epochs"].append(epoch)
            history["val_loss"].append(metrics["objective_mse"])
            history["val_mae"].append(metrics["mae"])
            history["val_r2"].append(metrics["r2"])
            history["val_regional_attention_entropy"].append(
                regional_attention["mean_normalized_entropy"]
            )
            history["val_regional_top_tile_mass"].append(regional_attention["mean_top_tile_mass"])
            history["val_local_attention_entropy"].append(
                local_attention["mean_normalized_entropy"]
            )
            history["val_local_top_tile_mass"].append(local_attention["mean_top_tile_mass"])
            improved_mse = (
            ...
            message += (
                ...
                f"R² {metrics['r2']:.3f} | 4x4 H "
                f"{regional_attention['mean_normalized_entropy']:.3f} | 5x5 H "
                f"{local_attention['mean_normalized_entropy']:.3f} | patience "
