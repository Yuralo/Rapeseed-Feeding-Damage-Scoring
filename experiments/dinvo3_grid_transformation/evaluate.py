"""Evaluate one DINOv3 regression checkpoint."""
...
def run(config: Config, checkpoint_path: str | Path, output_dir=None) -> dict:
    ...
    write_json(
        destination / "environment.json", environment_info(device, REPOSITORY_ROOT)
    )
...
def main(argv=None) -> None:
    ...
    report = run(
        load_config(arguments.config), arguments.checkpoint, arguments.output_dir
    )
