"""Scores, deterministic split, cached grid crops, and experiment loaders."""
...
def augment(image: Image.Image, config: Config) -> Image.Image:
    ...
    if strength:
        image = ImageEnhance.Brightness(image).enhance(uniform(1 - strength, 1 + strength))
        image = ImageEnhance.Contrast(image).enhance(uniform(1 - strength, 1 + strength))
        image = ImageEnhance.Color(image).enhance(uniform(1 - strength, 1 + strength))
    return image
...
class PlantDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> dict[str, object]:
        ...
        try:
            ...
            raise RuntimeError(
                f"Grid crop failed for {filename!r} at {source}. "
                f"Failure details were appended to {failure_log}."
            ) from error
        if self.training:
            image = augment(image, self.config)
