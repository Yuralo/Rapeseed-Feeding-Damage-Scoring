"""Train normalized DINOv3 regression with all-block LoRA and gated patch attention."""
...
from .model import DinoV3LoRAPatchAttentionRegressor, make_optimizer
...
def run(config: Config, resume: str | Path | None = None) -> dict:
    ...
    model = DinoV3LoRAPatchAttentionRegressor(config).to(device)
    ...
    for epoch in range(start_epoch, config.training.epochs + 1):
        ...
        if should_evaluate:
            ...
            metrics.update(
                {f"attention_{key}": value for key, value in attention_metrics.items()}
            )
            history["val_epochs"].append(epoch)
            history["val_loss"].append(metrics["objective_mse"])
            history["val_mae"].append(metrics["mae"])
            history["val_r2"].append(metrics["r2"])
            history["val_attention_entropy"].append(
                attention_metrics["mean_normalized_entropy"]
            )
            history["val_attention_top_10_mass"].append(
                attention_metrics["mean_top_10_percent_mass"]
            )
            improved = (
            ...
            message += (
                ...
                f" | attn H {attention_metrics['mean_normalized_entropy']:.3f}"
                f" | top10 {attention_metrics['mean_top_10_percent_mass']:.3f}"
