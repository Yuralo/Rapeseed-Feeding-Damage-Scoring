...
class DinoV3LoRAPatchAttentionRegressor(nn.Module):
...
def make_optimizer(model: DinoV3LoRAPatchAttentionRegressor, config: Config):
    ...
    for name, parameter in model.named_parameters():
        ...
        scope = (
            "head"
            if name.startswith(("regression_head.", "patch_attention."))
            else "adapter"
        )
