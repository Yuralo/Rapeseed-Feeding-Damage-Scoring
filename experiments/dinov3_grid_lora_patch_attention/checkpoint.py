"""Compact checkpoints for LoRA plus gated patch attention."""
...
EXPERIMENT_ID = "dinov3_grid_lora_patch_attention"
...
def scaler_from(state: dict[str, Any]) -> TargetScaler:
    ...
    if not state.get("targets_normalized", True):
        raise ValueError("LoRA patch-attention checkpoints must use normalized targets")
...
def validate_for(state: dict[str, Any], config: Config) -> None:
    ...
    if not state.get("targets_normalized", True) or not config.data.normalize_targets:
        raise ValueError("This LoRA patch-attention experiment requires normalized targets")
