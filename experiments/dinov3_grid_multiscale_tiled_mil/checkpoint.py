"""Head-only checkpoint schema for multi-scale cached-feature MIL."""
...
EXPERIMENT_ID = "dinov3_grid_multiscale_tiled_mil"
...
def payload(
    *,
    ...
    return {
        ...
        "model_state_format": "multiscale_mil_head_only",
...
def validate_for(state: dict[str, Any], config: Config, feature_dim: int | None = None) -> None:
    ...
    if state.get("model_state_format") != "multiscale_mil_head_only":
        raise ValueError("Checkpoint is not a multi-scale MIL head-only checkpoint")
